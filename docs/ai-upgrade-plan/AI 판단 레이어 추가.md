...
- Anthropic API(코드에서 `fetch`로 호출) = 별도 Console 계정 + 토큰 과금
...
- Google 계정으로 [aistudio.google.com](https://aistudio.google.com) → "Get API key" (1분)
- 한국어 품질 우수, 응답 속도 빠름
...
| 파일 | 변경 내용 |
|---|---|
| `battery_pack_renderer.html` | ① 사이드바 하단에 AI 키 입력 섹션 추가<br>② script 로드 목록에 `ai-advisor.js` 추가 |
| `src/app-render.js` | ③ `_renderSVG()` 후보 0개 분기: AI 진단 비동기 호출 + 결과 삽입<br>④ Generate 완료 후 `warnBoundary()` 트리거 |
